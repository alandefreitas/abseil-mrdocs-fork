...
// The Abseil `status` library.
//
// This namespace holds `absl::Status`, the canonical `absl::StatusCode` error
// codes, `absl::StatusOr`, and the helper functions used to create, inspect,
// and propagate status values.
namespace absl {
...
///
/// In most cases, an `absl::Status` indicates a recoverable error, and the
/// purpose of signalling an error is to indicate what action to take in
/// response to that error. These error codes map to the proto RPC error codes
/// indicated in https://cloud.google.com/apis/design/errors.
///
...
enum class StatusCode : int {
  /// (gRPC code "OK") does not indicate an error; this value is returned on
  /// success. It is typical to check for this value before proceeding on any
  /// given call across an API or RPC boundary. To check this value, use the
  /// `absl::Status::ok()` member function rather than inspecting the raw code.
  kOk = 0,

  /// (gRPC code "CANCELLED") indicates the operation was cancelled,
  /// typically by the caller.
  kCancelled = 1,

  /// (gRPC code "UNKNOWN") indicates an unknown error occurred. In
  /// general, more specific errors should be raised, if possible. Errors raised
  /// by APIs that do not return enough error information may be converted to
  /// this error.
  kUnknown = 2,

  /// (gRPC code "INVALID_ARGUMENT") indicates the caller
  ...
  /// (gRPC code "DEADLINE_EXCEEDED") indicates a deadline
  ...
  /// (gRPC code "NOT_FOUND") indicates some requested entity (such as
  ...
  /// (gRPC code "ALREADY_EXISTS") indicates that the entity a
  /// caller attempted to create (such as a file or directory) is already
  /// present.
  kAlreadyExists = 6,

  /// (gRPC code "PERMISSION_DENIED") indicates that the caller
  ...
  /// (gRPC code "RESOURCE_EXHAUSTED") indicates some resource
  /// has been exhausted, perhaps a per-user quota, or perhaps the entire file
  /// system is out of space.
  kResourceExhausted = 8,

  /// (gRPC code "FAILED_PRECONDITION") indicates that the
  ...
  /// (gRPC code "ABORTED") indicates the operation was aborted,
  ...
  /// (gRPC code "OUT_OF_RANGE") indicates the operation was
  ...
  /// (gRPC code "UNIMPLEMENTED") indicates the operation is not
  /// implemented or supported in this service. In this case, the operation
  /// should not be re-attempted.
  kUnimplemented = 12,

  /// (gRPC code "INTERNAL") indicates an internal error has occurred
  /// and some invariants expected by the underlying system have not been
  /// satisfied. This error code is reserved for serious errors.
  kInternal = 13,

  /// (gRPC code "UNAVAILABLE") indicates the service is currently
  ...
  /// (gRPC code "DATA_LOSS") indicates that unrecoverable data loss or
...
/// @return The status code name, or "" if `code` is unknown.
std::string StatusCodeToString(StatusCode code);

/// Same as StatusCodeToString(), but returns a string_view.
///
/// @param code The status code to convert.
/// @return A view of the status code name, or "" if `code` is unknown.
absl::string_view StatusCodeToStringView(StatusCode code);

/// Streams StatusCodeToString(code) to `os`.
///
/// @param os The output stream to write to.
/// @param code The status code to stream.
/// @return A reference to `os`.
std::ostream& operator<<(std::ostream& os, StatusCode code);

/// An enumerated type indicating how `absl::Status::ToString()` should
/// construct the output string for a non-ok status.
...
///
/// @param lhs The mode to update in place.
/// @param rhs The right-hand operand.
...
class ABSL_ATTRIBUTE_TRIVIAL_ABI Status final {
 public:
  ...
  ///
  /// @param code The canonical error code for the status.
  /// @param msg The UTF-8 error message.
  /// @param loc The source location of the callsite.
  ...
  ///
  /// The `loc` will be appended to the location chain of the new status, iff the
  /// `base_status` is not ok and has non-empty msg.
  ///
  /// @param base_status The status to copy.
  /// @param loc The source location to append to the location chain.
  ...
  ///
  /// @param base_status The status to move from.
  /// @param loc The source location to append to the location chain.
  ...
  /// Copy constructor.
  ///
  /// @param x The status to copy from.
  Status(const Status&);
  ...
  ///
  /// @param x The status to move from.
  ...
  ///
  /// @param new_status The status to apply if `*this` is currently OK.
  void Update(const Status& new_status);
#ifndef SWIG
  /// Updates the existing status by moving from `new_status` if `this->ok()`.
  ///
  /// @param new_status The status to move from if `*this` is currently OK.
  ...
  ///
  /// @return A view of the error message, which may be empty.
  absl::string_view message() const;
#endif  // SWIG

  /// Compares two statuses for equality.
  ///
  /// @param lhs The left-hand status.
  /// @param rhs The right-hand status.
  /// @return `true` if the two statuses are equal.
  ...
  /// @param lhs The left-hand status.
  /// @param rhs The right-hand status.
  /// @return `true` if the two statuses differ.
  friend bool operator!=(const Status&, const Status&);
  ...
  ///
  /// @param mode Controls which extra data is included in the output.
  ...
  ///
  /// @param sink The sink to append the string representation to.
  ...
  /// Ignores any errors.
  ///
  /// This method does nothing except potentially suppress complaints from any
  /// tools that are checking that errors are not dropped on the floor.
  ...
  ///
  /// @param type_url The unique key identifying the payload.
  /// @return The payload if present, or `std::nullopt` otherwise.
  ...
  ///
  /// @param type_url The unique key identifying the payload.
  /// @param payload The payload data to store.
  void SetPayload(absl::string_view type_url, absl::Cord payload);

  /// Erases the payload corresponding to the `type_url` key.
  ///
  /// @param type_url The unique key identifying the payload to erase.
  /// @return `true` if the payload was present.
  ...
  ///
  /// @param visitor The callable invoked for each stored payload.
  void ForEachPayload(
  ...
  ///
  /// @return A span over the stored source locations.
  ...
  ///
  /// @param loc The source location to append.
  ...
  ///
  /// @param loc The source location to append.
  /// @return A copy of this status with `loc` appended.
  ...
  ///
  /// @param loc The source location to append.
  /// @return An rvalue reference to `*this`.
 ...
 private:
  ...
  ///
  /// @param inlined_rep The inlined representation carrying the error code.
  /// @param msg The error message.
  /// @param loc The source location of the callsite.
  /// @return The representation for the resulting status.
...
///
/// Prefer usage of `absl::OkStatus()` when constructing such an OK status.
///
/// @return An OK status.
Status OkStatus();
...
/// @return `true` if `status` matches the associated error code.
...
/// @param loc The source location of the callsite.
/// @return A status with the `kAborted` error code.
...
/// @param message The error message.
/// @param loc The source location of the callsite.
/// @return A status with the `kAlreadyExists` error code.
...
/// @param message The error message.
/// @param loc The source location of the callsite.
/// @return A status with the `kCancelled` error code.
...
/// @param message The error message.
/// @param loc The source location of the callsite.
/// @return A status with the `kDataLoss` error code.
...
/// @param message The error message.
/// @param loc The source location of the callsite.
/// @return A status with the `kDeadlineExceeded` error code.
...
/// @param message The error message.
/// @param loc The source location of the callsite.
/// @return A status with the `kFailedPrecondition` error code.
...
/// @param message The error message.
/// @param loc The source location of the callsite.
/// @return A status with the `kInternal` error code.
...
/// @param message The error message.
/// @param loc The source location of the callsite.
/// @return A status with the `kInvalidArgument` error code.
...
/// @param message The error message.
/// @param loc The source location of the callsite.
/// @return A status with the `kNotFound` error code.
...
/// @param message The error message.
/// @param loc The source location of the callsite.
/// @return A status with the `kOutOfRange` error code.
...
/// @param message The error message.
/// @param loc The source location of the callsite.
/// @return A status with the `kPermissionDenied` error code.
...
/// @param message The error message.
/// @param loc The source location of the callsite.
/// @return A status with the `kResourceExhausted` error code.
...
/// @param message The error message.
/// @param loc The source location of the callsite.
/// @return A status with the `kUnauthenticated` error code.
...
/// @param message The error message.
/// @param loc The source location of the callsite.
/// @return A status with the `kUnavailable` error code.
...
/// @param message The error message.
/// @param loc The source location of the callsite.
/// @return A status with the `kUnimplemented` error code.
...
/// @param message The error message.
/// @param loc The source location of the callsite.
/// @return A status with the `kUnknown` error code.
...
/// references.
///
/// @param error_number An `errno` value.
...
/// @param message The error message.
/// @param loc The source location of the callsite.
/// @return A status derived from `error_number`.
...
///
/// It is provided only for efficiency, given that message-less kCancelled
/// errors are common in the infrastructure.
///
/// @return A `kCancelled` status with an empty message.
inline Status CancelledError() { return Status(absl::StatusCode::kCancelled); }

/// Retrieves a message's status as a null terminated C string.
///
/// The lifetime of this string is tied to the lifetime of the status object
/// itself.
///
...
///
/// @param status The status whose message is retrieved.
/// @return A null-terminated C string for the status message.
