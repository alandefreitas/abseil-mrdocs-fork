...
///
/// @param source The escaped string to unescape.
/// @param dest The output string that receives the unescaped result.
/// @param error Receives the first error encountered, or `nullptr` to disable
///   error reporting.
...
/// Encodes a `src` string into a base64-encoded string with padding
...
///
/// @param src The string to encode.
/// @param dest The output string that receives the base64-encoded result.
[[deprecated(
...
/// outputs '-' instead of '+' and '_' instead of '/', and does not pad the
/// output. This function conforms with RFC 4648 section 5 (base64url).
///
/// @param src The string to encode.
/// @return The web-safe base64-encoded string.
std::string WebSafeBase64Escape(absl::string_view src);
/// Deprecated overload of `WebSafeBase64Escape()` that writes to a `dest`
/// buffer.
///
/// @param src The string to encode.
/// @param dest The output string that receives the base64-encoded result.
...
/// @param dest The output string that receives the decoded bytes.
/// @return `true` on success, `false` if `src` contains invalid characters.
...
/// identically.
///
/// @param src The web-safe base64-encoded string to decode.
/// @param dest The output string that receives the decoded bytes.
/// @return `true` on success, `false` if `src` contains invalid characters.
...
/// true on success.
///
/// @param hex The hexadecimal-encoded string to decode.
/// @param bytes The output string that receives the decoded bytes.
/// @return `true` on success, `false` if `hex` is not valid hexadecimal data.
...
///
/// @param from The hexadecimal-encoded string to decode.
...
/// `2*from.size()`.
///
/// @param from The binary data to encode.
...
///
/// @param input The URL component to escape.
/// @return The escaped string.
...
/// unescape '+' to ' '.
///
/// @param input The percent-encoded string to unescape.
/// @return The unescaped string, or `std::nullopt` on malformed input.
