...
///
/// Such implicit cast chaining may be useful within template logic. This
/// overload participates in overload resolution when `To` is not a view type
/// and not a reference type.
/// @param to The value to convert to `To`.
/// @return The value of `to`, converted to `To`.
...
/// Overload of `implicit_cast()` for view types (such as `std::string_view`),
/// which participates in overload resolution when `To` is a non-reference
/// view type.
/// @param to The value to convert to `To`.
/// @return The value of `to`, converted to `To`.
template <typename To>
...
/// Overload of `implicit_cast()` for reference types, which participates in
/// overload resolution when `To` is a reference type.
/// @param to The value to convert to `To`.
/// @return The value of `to`, converted to `To`.
template <typename To>
...
template <typename Dest, typename Source,
          std::enable_if_t<sizeof(Dest) == sizeof(Source) &&
                           ...
                           int> = 0>
#if ABSL_HAVE_BUILTIN(__builtin_bit_cast)
/// Bitwise-casts `source` to `Dest` using the compiler builtin.
/// @param source The value whose representation is reinterpreted as `Dest`.
/// @return A `Dest` value with the same bit representation as `source`.
inline constexpr Dest bit_cast(const Source& source) {
  return __builtin_bit_cast(Dest, source);
}
#else  // ABSL_HAVE_BUILTIN(__builtin_bit_cast)
/// Bitwise-casts `source` to `Dest` by copying its object representation.
/// @param source The value whose representation is reinterpreted as `Dest`.
/// @return A `Dest` value with the same bit representation as `source`.
...
/// @param f The source pointer to downcast.
/// @return `f`, downcast to `To`, or null if `f` is null.
...
/// @param f The source reference to downcast.
/// @return `f`, downcast to `To`.
template <typename To, typename From>
[[nodiscard]]
