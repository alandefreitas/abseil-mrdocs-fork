...
/// This structure contains the collection of callbacks for changing the
/// behavior of the usage reporting routines in Abseil Flags.
struct FlagsUsageConfig {
  ...
  /// contains_helpshort_flags("path/to/my/code.cc") returns true, invoking
  /// the program with --helpshort will include information about --my_flag
  ...
  /// "path/to/my/code.cc" defines the flag "--my_flag", and
  /// contains_helppackage_flags("path/to/my/code.cc") returns true, invoking
  /// the program with --helppackage will include information about
  /// --my_flag in the program output.
  flags_internal::FlagKindFilter contains_helppackage_flags;

  /// Generates string containing program version. This is the string
  /// reported when user specifies --version in a command line.
  std::function<std::string()> version_string;

  /// Normalizes the filename specific to the build system/filesystem used.
  /// This routine is used when we report the information about the flag
  /// definition location. For instance, if your build resides at some
  /// location you do not want to expose in the usage output, you can trim
  /// it to show only relevant part.
  /// For example:
...
/// Sets the usage reporting configuration callbacks. If any of the
/// callbacks are not set in usage_config instance, then the default value
/// of the callback is used.
///
/// @param usage_config The usage reporting configuration to install.
