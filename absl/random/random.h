...
///
/// Typically, you use a bit generator in combination with a distribution to
...
class BitGen : private random_internal::NonsecureURBGBase<
 ...
 public:
  /// The type of the values produced by this bit generator.
  using result_type = typename Base::result_type;

  /// Inherits the base constructors.
  ///
  /// `BitGen()` default-constructs the generator, and
  /// `BitGen(SeedSequence seed_seq)` seeds it from a seed sequence. Copy is
  /// disallowed; move is allowed.
  using Base::Base;
  /// Inherits the base assignment operators.
  using Base::operator=;
  ...
  ///
  /// Advances the internal state by `num` times, and discards the intermediate
  /// results.
  using Base::discard;

  /// Invokes the URBG, returning a generated value.
  using Base::operator();

  /// Compares two bit generators for equality.
  using Base::operator==;
  /// Compares two bit generators for inequality.
  using Base::operator!=;
};

/// An efficient random bit generator for performance-sensitive use cases.
///
/// `absl::InsecureBitGen` is recommended only for performance-sensitive use
/// cases where `absl::BitGen` is not satisfactory when compute-bounded by bit
/// generation costs.
///
/// Example:
///
...
class InsecureBitGen : private random_internal::NonsecureURBGBase<
 ...
 public:
  ...
  /// Inherits the base constructors.
  ///
  /// `InsecureBitGen()` default-constructs the generator, and
  /// `InsecureBitGen(SeedSequence seed_seq)` seeds it from a seed sequence.
